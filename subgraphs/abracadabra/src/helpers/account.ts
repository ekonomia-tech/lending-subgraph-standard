...
export function getOrCreateAccountInMarket(marketId: string, accountId: string): AccountInMarket {
  ...
  if (!acm) {
    ...
    acm.deposited = BigDecimal.zero()
    acm.borrowed = BigDecimal.zero()
    acm.lifetimeBorrowed = BigDecimal.zero()
    acm.lifetimeDeposited = BigDecimal.zero()
    acm.lifetimeLiquidated = BigDecimal.zero()
    acm.lifetimeRepaid = BigDecimal.zero()
    acm.lifetimeWithdrawn = BigDecimal.zero()
    acm.save()
  }
  return acm
}
